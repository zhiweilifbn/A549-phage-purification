...
#   ../output/soft_threshold_diagnostic.svg  (soft-thresholding power
#                                 diagnostic plot; the selected power
...
stopifnot(!is.na(SELECTED_BETA))
message("Selected soft-thresholding power (beta) = ", SELECTED_BETA,
        " (computed from the R-squared/connectivity criteria)")

# ---- export the soft-thresholding diagnostic plot (../output, same folder as other outputs) ----
svg(file.path(OUT_DIR, "soft_threshold_diagnostic.svg"), width = 10, height = 4.5)
par(mfrow = c(1, 2), mar = c(5, 5, 3, 1))
plot(sft_df$Power, sft_df$signed_Rsq, type = "n",
     xlab = "Soft Threshold (power)", ylab = expression(Signed~R^2),
     main = "Scale-Free Topology Fit")
text(sft_df$Power, sft_df$signed_Rsq, labels = sft_df$Power,
     col = ifelse(sft_df$Power == SELECTED_BETA, "red", "grey50"),
     font = ifelse(sft_df$Power == SELECTED_BETA, 2, 1))
abline(h = c(0.80, 0.85), col = c("blue", "darkgreen"), lty = 2)

plot(sft_df$Power, sft_df$mean_k, type = "n", log = "y",
     xlab = "Soft Threshold (power)", ylab = "Mean Connectivity (log)",
     main = "Mean Connectivity")
text(sft_df$Power, sft_df$mean_k, labels = sft_df$Power,
     col = ifelse(sft_df$Power == SELECTED_BETA, "red", "grey50"),
     font = ifelse(sft_df$Power == SELECTED_BETA, 2, 1))
abline(h = c(50, 200), col = "darkgreen", lty = 2)
dev.off()
message("Saved: soft_threshold_diagnostic.svg (selected power = ", SELECTED_BETA, " highlighted in red)")
